 ...
 * Redis对象操作实现
 * 
 * 一、object.c 做了什么？ 答:负责处理 robj 结构的增删改查 
 * - 将整数或字符串包装成 robj 结构, 然后返回给调用方
 * - 或者从 robj 结构中提取整数或字符串, 并将提取值返回给调用方
 * 
 *          integer    ------------
 *          --->       |          |
 * Caller   robj       |          |
 ...
 * 二、整数或字符串创建 robj 的规则
 *  1. 整数
 *      - 整数在 [0,10000] 范围内, 返回redis预先创建的 robj(共享内存), 不在新增
 *      - 整数在 [LONG_MIN,LONG_MAX] 范围, 以 INT 编码存入 robj
 *      - 整数 > LONG_MAX, 以 RAW 编码存入 robj
 * 
 *  2. 字符串
 *      - 字符串内容是整数, 转换类型后以 INT 编码存入 robj
 *      - 字符串长度 <= 39, 以 EMBSTR 编码存入 robj
 *      - 字符串长度 >  39, 以 RAW 编码存入 robj
 * 
 *  
 * 三、robj 生存周期
 *     1. 创建对象 (object.c完成)
 *     2. 操作对象
 *    (由 t_string.c, t_hash.c, t_list.c, t_zset.c, t_set.c 完成)
 *     3. 释放对象 (object.c完成)
 * 
 * 
 * 四、函数按功能分类
